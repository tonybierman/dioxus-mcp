...
pub(super) const CORE_SCREEN: &str = r#"  Screen:
    ...
    structural_layout_kinds: [holy_grail, bento_grid, masonry, full_bleed, sticky_sidebar, mega_menu, drawer, card_grid, editorial, hero_scroll, split_screen, scroll_sticky]  # registry-driven; get_registry is authoritative
    client_crud_fields:
    ...
    example:
      screens:
        ...
        - name: DashboardScreen
          route: /dashboard
          # Structural layout from the registry library — a Tailwind-classed
          # holy-grail shell (header / sidebar / main / aside / footer).
          template:
            kind: holy_grail
"#;
...
pub(super) const CORE_SERVER_FN: &str = r#"  ServerFn:
    ...
    fields:
      ...
      - {name: extractors, type: "ArgDef[] — Axum-style request extractors. Each {name, type} entry lands in the `#[get/post(...)]` attribute's argument list only — the Dioxus 0.7.9 verb-macro binds the name into the fn scope itself, and adding it to the fn signature would break `FromRequest` for the body tuple. Example: extractors: [{name: cookies, type: \"TypedHeader<Cookie>\"}] emits `#[get(\"/api/board\", cookies: TypedHeader<Cookie>)]` and the body can use `cookies` directly. You're responsible for adding the relevant extractor crates (axum_extra, axum::headers, tower_cookies, …) to Cargo.toml so the type resolves.", required: false}
      - {name: auth_required, type: "bool (default false). When true the scaffolder injects the canonical cookie-authed prologue: ensures a `cookies: TypedHeader<Cookie>` extractor is declared (auto-prepended when missing), pulls the session id out of the named cookie, and maps the missing-cookie case to `ServerFnError::new(\"not logged in\")`. A `// TODO touch_session(session_id).await?` marker is left in the body so the caller can wire it to their own session store. You still need axum-extra (with the `typed-header` and `cookie` features) in Cargo.toml. WRITE-SIDE COUNTERPART: for the login fn that has to actually emit `Set-Cookie`, call `FullstackContext::current().add_response_header(\"set-cookie\", ...)` from inside the fn body — `search_docs query: \"set cookie from server fn\"` returns a curated snippet.", required: false}
...
pub(super) const CORE_RESOURCE: &str = r#"  Resource:
    ...
    fields:
      ...
      - {name: auth_required, type: "bool (default false). When true every synthesized server fn (list/get/create/update/delete) receives the same cookie-authed prologue as `ServerFn { auth_required: true }`: a `cookies: TypedHeader<Cookie>` extractor is added and the session id is extracted, mapping the missing-cookie case to `ServerFnError::new(\"not logged in\")`. Pair with the auth extension's session_state primitive on the client side. WRITE-SIDE: for the login fn that has to emit Set-Cookie, see `search_docs query: \"set cookie from server fn\"`.", required: false}
      - {name: session_cookie, type: "string (default \"session_id\"). Cookie name the auth prologue reads. Only consulted when `auth_required: true`.", required: false}
    example:
      resources:
        - name: Product
